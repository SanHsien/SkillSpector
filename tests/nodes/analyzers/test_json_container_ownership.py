...
@pytest.mark.parametrize(
    ...
    # Oversized payloads would otherwise become the test id, which pytest copies into
    # PYTEST_CURRENT_TEST; Windows rejects environment values over 32,767 characters.
    ids=lambda content: content if len(content) <= 200 else f"payload-{len(content)}-chars",
)
